...
if __name__ == '__main__':
    ...
    # Use a plain autoencoder (ChromatinAE) mirroring the chromatin_vae layout
    # Encoder: in -> 2h -> h -> h -> dim; Decoder: dim -> h -> 2h -> out
    # Keep hidden_dim comparable to previous (dim*2) for capacity parity
    model = ChromatinAE(input_dim=hic.shape[-1], hidden_dim=dim * 2, embed_dim=dim,
                        out_dim=cell_attributes.shape[-1], dropout=0.0, layer_norm=True).to(device)
