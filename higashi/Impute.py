...
def impute_process(config_path, model, name, mode, cell_start, cell_end, sparse_path, weighted_info=None):
    ...
    # configurable predict batch size (default 1e5)
    impute_bs = int(config.get('impute_batch_size', int(1e5)))
    ...
    with torch.no_grad():
        ...
        for i in cell_list:
            ...
            big_samples[:, 0] = cell
            proba = model.predict(big_samples, big_samples_chrom, verbose=False, batch_size=impute_bs,
