...
# A simple deterministic autoencoder mirroring the layer layout of the
# chromatin VAE (without stochastic reparameterization). This is used by
# the co-assay pretrain when we want a plain AE rather than a VAE-like head.
class ChromatinAE(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, embed_dim: int,
                 out_dim: int, dropout: float = 0.0, layer_norm: bool = True):
        super().__init__()
        ...
        # Encoder: in -> 2h -> h -> h -> embed
        self.e1 = nn.Linear(input_dim, 2 * hidden_dim).to(device, non_blocking=True)
        self.e2 = nn.Linear(2 * hidden_dim, hidden_dim).to(device, non_blocking=True)
        self.e3 = nn.Linear(hidden_dim, hidden_dim).to(device, non_blocking=True)
        self.e_out = nn.Linear(hidden_dim, embed_dim).to(device, non_blocking=True)

        # Decoder: embed -> h -> 2h -> out_dim (linear)
        self.d1 = nn.Linear(embed_dim, hidden_dim).to(device, non_blocking=True)
        self.d2 = nn.Linear(hidden_dim, 2 * hidden_dim).to(device, non_blocking=True)
        self.d_out = nn.Linear(2 * hidden_dim, out_dim).to(device, non_blocking=True)

        self.dropout = nn.Dropout(dropout) if dropout and dropout > 0.0 else None
        self.ln_e2 = nn.LayerNorm(hidden_dim).to(device, non_blocking=True) if layer_norm else None
        self.ln_emb = nn.LayerNorm(embed_dim).to(device, non_blocking=True) if layer_norm else None

    def encode(self, x):
        h1 = F.leaky_relu(self.e1(x))
        h2 = F.leaky_relu(self.e2(h1))
        h3 = F.leaky_relu(self.e3(h2) + h2)  # small residual as in chromatin_vae
        if self.ln_e2 is not None:
            h3 = self.ln_e2(h3)
        z = self.e_out(h3)
    ...
    def decode(self, z):
        h1 = F.leaky_relu(self.d1(z))
        h2 = F.leaky_relu(self.d2(h1))
        y = self.d_out(h2)  # regression to continuous ATAC target
        return y
...
class Hyper_SAGNN(nn.Module):
    def __init__(
            ...
            chrom_num=1,
            enable_dual_coassay: bool = False,
            atac_dropout_p: float = 0.0):
        super().__init__()
        ...
        self.enable_dual_coassay = enable_dual_coassay

        # Optional ATAC prediction head (per-cell, per-bin). Use cell+bin concat features.
        if self.enable_dual_coassay:
            self.pff_atac = FeedForward([d_model * 2, int(d_model / 2), 1])
            self.atac_dropout_p = float(atac_dropout_p) if atac_dropout_p is not None else 0.0
            self.dropout_atac = nn.Dropout(self.atac_dropout_p) if self.atac_dropout_p > 0 else None
    ...
    def forward(self, x, x_chrom, mask=None, chroms_in_batch=None):
        ...
        if not self.only_distance:
            ...
            if self.enable_dual_coassay:
                # still return placeholders for shape-consistency in caller
                dummy = torch.zeros((len(x), 1), dtype=torch.float, device=device)
                return distance_proba2, distance_proba3, distance_proba, dummy, dummy
            return distance_proba2, distance_proba3, distance_proba

        # Optional ATAC head using cell–bin dynamic embeddings to capture per-cell shape
        if self.enable_dual_coassay:
            # Use dynamic features after attention so bins are conditioned on the cell token
            # dynamic: [batch, seq_len, d_model]
            cell_feat_dyn = dynamic[:, 0, :]
            if self.dropout_atac is not None:
                cell_feat_dyn = self.dropout_atac(cell_feat_dyn)
            bin1_feat_dyn = dynamic[:, 1, :]
            bin2_feat_dyn = dynamic[:, 2, :]
            atac_b1 = self.pff_atac(torch.cat([cell_feat_dyn, bin1_feat_dyn], dim=-1))
            atac_b2 = self.pff_atac(torch.cat([cell_feat_dyn, bin2_feat_dyn], dim=-1))
            return output_mean, output_var, output_proba, atac_b1, atac_b2
        else:
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for j in func1(math.ceil(len(input) / batch_size)):
                    ...
                    # Call model; handle dual-head tuple size flexibly
                    ret = self(x, x_chrom)
