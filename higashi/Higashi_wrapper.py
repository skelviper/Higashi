...
class Higashi():
    ...
    def fetch_info_from_config(self):
        ...
        # Allow capping batch size via config for better GPU utilization
    ...
    def prep_model(self):
        ...
        self.batch_size = min(
            int(256 * max((1000000 / self.res), 1) * max(self.num[0] / 6000, 1)),
            self.max_batch_size)
        ...
        # Normalize coverage-like cell factor to mean 1 to avoid scale dominating ZINB mean
        self.cell_feats1_norm = self.cell_feats1 / (self.cell_feats1.mean() + 1e-8)
        ...
        self.node_embedding_init.wstack[0].fit(embeddings_initial[0], 300,
                                               ...
                                               batch_size=1024)
        enable_dual = bool(self.config.get('coassay_imputation', False)) and \
                       (self.config.get('coassay_imputation_mode', 'dual_coupled') in ['dual_coupled', 'dual_head'])
        self.dual_coassay = enable_dual
        self.coassay_loss = self.config.get('coassay_loss', 'mse')
        self.coassay_loss_weight = float(self.config.get('coassay_loss_weight', 1.0))
        self.coassay_gate_gamma = float(self.config.get('coassay_gate_gamma', 0.0))  # gating strength; 0 disables
        # Plan A options
        self.coassay_quantile_tau = float(self.config.get('coassay_quantile_tau', 0.3))
        self.coassay_l1_lambda = float(self.config.get('coassay_l1_lambda', 0.0))
        self.coassay_atac_dropout = float(self.config.get('coassay_atac_dropout', 0.0))
        ...
        self.higashi_model = Hyper_SAGNN(
            ...
            enable_dual_coassay=self.dual_coassay,
            atac_dropout_p=self.coassay_atac_dropout).to(device, non_blocking=True)
    ...
    def _prepare_atac_arrays(self):
        """Load ATAC arrays per chromosome into memory with configurable normalization.
        Requires: self.chrom_list, self.data_dir, self.config, self.cell_num
        Produces: self._atac_by_chrom: dict chrom -> np.ndarray [cells, bins_on_chrom]
        and self._atac_bin_mask: dict chrom -> boolean mask over full bins

        Normalization (configurable via config['coassay_label_norm']):
          - 'per_bin_z'  (default for mse/quantile): z-score per bin across cells (previous behavior)
          - 'per_cell_z': z-score per cell across bins (preserves per-bin baselines across cells)
          - 'none'       (recommended for bce): raw values; optionally log1p if config['coassay_label_log1p']=true
        """
        signal_names = self.config.get('coassay_signal', [])
        if len(signal_names) == 0:
            raise RuntimeError('No coassay_signal configured')
        import h5py
        f = h5py.File(os.path.join(self.data_dir, 'sc_signal.hdf5'), 'r')
        g = f[signal_names[0]]
        # decode chrom labels
        try:
        ...
        label_norm = self.config.get('coassay_label_norm', None)
        # If using BCE, default to 'none' to avoid zero-centering across cells
        if self.config.get('coassay_loss', 'mse') == 'bce' and (label_norm is None or label_norm == 'per_bin_z'):
            label_norm = 'none'
        if label_norm is None:
            label_norm = 'per_bin_z'  # backward-compatible default
        log1p_flag = bool(self.config.get('coassay_label_log1p', False))
        for chrom in self.chrom_list:
            mask = (chrom_arr == chrom)
            self._atac_bin_mask[chrom] = mask
            # stack cells
            mats = []
            for ci in range(self.cell_num):
                mats.append(np.array(g[str(ci)])[mask])
            A = np.stack(mats, axis=0).astype('float32')
            if log1p_flag:
                A = np.log1p(A)
            if label_norm == 'per_bin_z':
                # Standardize per-bin across cells
                try:
                    from sklearn.preprocessing import StandardScaler
                    A = StandardScaler(with_mean=True, with_std=True).fit_transform(A)
                except Exception:
                    pass
            elif label_norm == 'per_cell_z':
                # Standardize per-cell across bins
                mu = A.mean(axis=1, keepdims=True)
                sigma = A.std(axis=1, keepdims=True) + 1e-6
                A = (A - mu) / sigma
            elif label_norm == 'none':
                # No scaling; keep raw (or log1p) values
                A = A
            else:
                # Fallback to no-op if unknown
                A = A
            self._atac_by_chrom[chrom] = A
        self._atac_h5 = f  # keep handle

    def _gather_atac_labels(self, batch_x, batch_chrom):
        """Return (y1, y2) tensors of shape [N,1] with standardized ATAC labels for bin1/bin2.
        Accepts torch.Tensor or np.ndarray for both inputs.
        """
        # batch_x: [N, 3] with 1-based ids; batch_chrom: [N] 0-based chrom indices
        if isinstance(batch_x, torch.Tensor):
            x = batch_x.detach().cpu().numpy()
        else:
            x = np.asarray(batch_x)
        if isinstance(batch_chrom, torch.Tensor):
            batch_chrom_np = batch_chrom.detach().cpu().numpy().reshape(-1)
        else:
            batch_chrom_np = np.asarray(batch_chrom).reshape(-1)
        y1 = np.zeros((len(x), 1), dtype='float32')
        y2 = np.zeros((len(x), 1), dtype='float32')
        for idx in range(len(x)):
            ci = int(x[idx, 0]) - 1  # cell id 0-based
            cidx = int(batch_chrom_np[idx])  # assumed 0-based chrom index
            chrom = self.chrom_list[cidx]
            # local bin index within chrom: global id minus cumulative offset
            b1 = int(x[idx, 1] - self.num_list[cidx] - 1)
            b2 = int(x[idx, 2] - self.num_list[cidx] - 1)
            A = self._atac_by_chrom[chrom]
            if 0 <= b1 < A.shape[1]:
                y1[idx, 0] = A[ci, b1]
            if 0 <= b2 < A.shape[1]:
                y2[idx, 0] = A[ci, b2]
        y1 = torch.from_numpy(y1).to(device, non_blocking=True)
        y2 = torch.from_numpy(y2).to(device, non_blocking=True)
        return y1, y2
    ...
    def forward_batch_hyperedge(self, batch_data, batch_weight, batch_chrom, batch_to_neighs, y,
        ...
        atac_y1 = atac_y2 = None
        if self.dual_coassay:
            pred, pred_var, pred_proba, atac_b1, atac_b2 = model(x, (batch_chrom, batch_to_neighs), chroms_in_batch=chroms_in_batch + 1)
            # Optional gating: use normalized ATAC labels to bias Hi-C logits slightly
            if self.coassay_gate_gamma > 0:
                if not hasattr(self, '_atac_ready'):
                    try:
                        self._prepare_atac_arrays()
                        self._atac_ready = True
                    except Exception as e:
                        print('Dual coassay enabled but failed to prepare ATAC arrays:', e)
                        self._atac_ready = False
                if getattr(self, '_atac_ready', False):
                    atac_y1, atac_y2 = self._gather_atac_labels(x, batch_chrom)
                    gate = self.coassay_gate_gamma * (atac_y1.view(-1, 1) + atac_y2.view(-1, 1)) * 0.5
                    pred = pred + gate
        else:
        ...
        if self.mode == 'classification':
            ...
            pred = F.softplus(pred)
            # Use normalized per-cell factor to stabilize scale
            extra = (getattr(self, 'cell_feats1_norm', self.cell_feats1)[batch_data[:, 0] - 1]).view(-1, 1)
            pred = pred * extra
            ...
            print("wrong mode")
            raise EOFError
        # Optional: ATAC auxiliary loss (dual head)
        if self.dual_coassay:
            # Lazy-load ATAC labels if needed
            if not hasattr(self, '_atac_ready'):
                try:
                    self._prepare_atac_arrays()
                    self._atac_ready = True
            ...
            if getattr(self, '_atac_ready', False):
                ...
                if self.coassay_loss == 'zinb':
                    ...
                    thr = float(self.config.get('coassay_bce_threshold', 0.0))
                    y1_bin = (atac_y1 > thr).float().view(-1)
                    y2_bin = (atac_y2 > thr).float().view(-1)
                    # Auto-balance pos_weight if requested
                    pw_cfg = self.config.get('coassay_bce_pos_weight', None)
                    auto_pw = False
                    if isinstance(pw_cfg, str):
                        auto_pw = (pw_cfg.lower() == 'auto')
                    elif isinstance(pw_cfg, (int, float)):
                        auto_pw = (float(pw_cfg) <= 0)
                    if auto_pw:
                        with torch.no_grad():
                            pos_cnt = y1_bin.sum() + y2_bin.sum()
                            tot = y1_bin.numel() + y2_bin.numel()
                            neg_cnt = max(tot - pos_cnt, torch.tensor(1.0, device=device))
                            # pos_weight ≈ neg/pos，裁剪到合理范围
                            raw_pw = (neg_cnt / torch.clamp(pos_cnt, min=1.0)).item()
                            max_pw = float(self.config.get('coassay_bce_pos_weight_max', 200.0))
                            pos_w_val = float(min(max(raw_pw, 1.0), max_pw))
                            self._last_atac_pos_rate = float((pos_cnt / tot).item())
                            self._last_atac_pos_weight = pos_w_val
                        pos_w = torch.as_tensor(pos_w_val, device=device)
                    else:
                    ...
                    with torch.no_grad():
                        p1 = (logits1 > 0).float()
                        p2 = (logits2 > 0).float()
                        acc1 = (p1 == y1_bin).float().mean().item()
                        acc2 = (p2 == y2_bin).float().mean().item()
                        self._last_atac_acc = float(0.5 * (acc1 + acc2))
                ...
                if lam > 0:
                    reg_mode = self.config.get('coassay_l1_mode', 'auto')
                    if self.coassay_loss == 'bce':
                        # Prefer penalizing probability mass on negatives, not logits around 0 (which would push prob→0.5)
                        prob1 = torch.sigmoid(atac_b1).view(-1)
                        prob2 = torch.sigmoid(atac_b2).view(-1)
                        # y1_bin/y2_bin are already available in BCE branch
                        if reg_mode in ('neg_prob', 'auto'):
                            reg = (prob1 * (1.0 - y1_bin)).mean() + (prob2 * (1.0 - y2_bin)).mean()
                        elif reg_mode == 'prob':
                            reg = prob1.mean() + prob2.mean()
                        elif reg_mode == 'logit':
    ...
    def train_epoch(self, training_data_generator, optimizer_list, train_pool, train_p_list):
        ...
        prefetch_batches = int(min(batch_num, max(self.cpu_num, 1) * 2))
        submitted = 0
        y_list, w_list, pred_list = [], [], []
        bar = trange(batch_num * self.collect_num, desc=' - (Training) ', leave=False, )
        
        while len(train_p_list) < prefetch_batches and submitted < batch_num:
        ...
        for p in as_completed(train_p_list):
            ...
            for j in range(self.collect_num):
                ...
                bar.update(n=1)
                last_atac = getattr(self, '_last_atac_loss', 0.0)
    ...
    def impute_atac_dual(self, save_dir=None, stage=3):
        """Impute ATAC (per cell x per chrom bins) using the dual head.
        Saves per-chrom arrays by default as z-scores:
          - imputed_atac_{chrom}.npy shape [n_cells, n_bins]

        If configured, also writes calibrated variants:
          - imputed_atac_{chrom}_prob.npy  -> sigmoid(k * z) in [0, 1]
          - imputed_atac_{chrom}_raw.npy   -> de-standardized counts (approx.)

        Controls (optional in config.JSON):
          - coassay_atac_output_prob: bool, default true
          - coassay_atac_prob_slope: float, default 1.0 (larger => sharper)
          - coassay_atac_output_raw: bool, default false
        """
        if not getattr(self, 'dual_coassay', False):
            raise RuntimeError('Dual coassay not enabled in config; cannot impute ATAC.')
        if save_dir is None:
            save_dir = self.temp_dir
        # Load a trained model checkpoint
        if stage == 3 and os.path.exists(self.save_path + "_stage3_model"):
            self.higashi_model = torch.load(self.save_path + "_stage3_model", map_location=self.current_device)
        elif os.path.exists(self.save_path + "_stage2_model"):
            self.higashi_model = torch.load(self.save_path + "_stage2_model", map_location=self.current_device)
        else:
            raise RuntimeError('Trained model checkpoint not found; run train_for_imputation_* first.')
        self.higashi_model.eval()
        device_local = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.higashi_model.to(device_local)
        # Use GraphSAGE dynamic pathway at inference if available (aligns with training)
        # Prepare utilities locally to avoid global side-effects
        try:
            from scipy.stats import norm as _norm
            from scipy.sparse import vstack as _vstack
            from sklearn.preprocessing import normalize as _sparse_l1_normalize
        except Exception:
            _norm = None
            _vstack = None
            _sparse_l1_normalize = None

        def _moving_avg(_adj, moving_range: int):
            if moving_range <= 0:
                return _adj
            if _norm is None or _vstack is None:
                return _adj
            adj_origin = _adj.copy()
            adj = _adj.copy()
            adj = adj * _norm.pdf(0)
            for i in range(moving_range * 2):
                before_list = [adj_origin[0, :]] * (i + 1) + [adj_origin[:-(i + 1), :]]
                adj_before = _vstack(before_list)
                after_list = [adj_origin[i + 1:, :]] + [adj_origin[-1, :]] * (i + 1)
                adj_after = _vstack(after_list)
                adj = adj + (adj_after + adj_before) * _norm.pdf((i + 1) / max(moving_range, 1))
            return adj

        dynamic_encoder = getattr(self.higashi_model.encode1, 'dynamic_nn', None)
        use_graphsage = dynamic_encoder is not None and hasattr(dynamic_encoder, 'fix_cell2')
        route_nn_list = [i + 1 for i in range(len(self.chrom_list))]
        local_transfer_range = int(self.config.get('local_transfer_range', 0))
        sparse_chrom_list = None
        if use_graphsage:
            sparse_path = os.path.join(self.temp_dir, "sparse_nondiag_adj_nbr_1.npy")
            if os.path.exists(sparse_path):
                try:
                    sparse_chrom_list = np.load(sparse_path, allow_pickle=True)
                    dynamic_encoder.start_fix()
                    # Use cached-feature path for inference
                    if hasattr(dynamic_encoder, 'forward_off_hook'):
                        dynamic_encoder.forward = dynamic_encoder.forward_off_hook
                except Exception:
                    use_graphsage = False
            else:
                use_graphsage = False
        ...
        n_cells = int(self.num[0])
        # Optional post-processing options
        want_prob = bool(self.config.get('coassay_atac_output_prob', True))
        prob_k = float(self.config.get('coassay_atac_prob_slope', 1.0))
        want_raw = bool(self.config.get('coassay_atac_output_raw', False))
        # If raw requested, we will compute per-chrom (mu, sigma) from sc_signal.hdf5
        h5 = None
        g = None
        chrom_arr = None
        if want_raw:
            try:
                import h5py
                h5 = h5py.File(os.path.join(self.data_dir, 'sc_signal.hdf5'), 'r')
                sig_names = self.config.get('coassay_signal', [])
                if len(sig_names) > 0 and sig_names[0] in h5:
                    g = h5[sig_names[0]]
                    try:
                        chrom_arr = g['bin']['chrom'].asstr()[...]
                    except AttributeError:
                ...
                g = None
        # Precompute per-chrom bin id arrays once
        bin_ids_all = [np.arange(num_list[j], num_list[j + 1]) + 1 for j in range(len(self.chrom_list))]

        for c, chrom in enumerate(self.chrom_list):
            start = int(num_list[c])
            end = int(num_list[c+1])
            n_bins = end - start
            out = np.zeros((n_cells, n_bins), dtype='float32')
            bins_global = np.arange(start, end, dtype=np.int64) + 1
            chrom_idx_arr = np.full(n_bins, c, dtype=np.int64)
            for cell in range(n_cells):
                cell_id = cell + 1
                # If GraphSAGE is available, precompute cached bin features for this cell across all chroms
                if use_graphsage and (sparse_chrom_list is not None) and (_sparse_l1_normalize is not None):
                    try:
                        cell_chrom_list = []
                        for jj in range(len(self.chrom_list)):
                            mtx = sparse_chrom_list[jj][cell]
                            adj = _moving_avg(mtx, local_transfer_range)
                            adj.data = np.log1p(adj.data)
                            adj = _sparse_l1_normalize(adj, norm='l1', axis=1).astype('float32')
                            Acoo = adj.tocoo()
                            indices = torch.from_numpy(np.asarray([Acoo.row, Acoo.col]))
                            vals = torch.from_numpy(Acoo.data)
                            cell_chrom_list.append([indices, vals, adj.shape])
                        dynamic_encoder.fix_cell2(cell_id, bin_ids_all, cell_chrom_list, local_transfer_range,
                                                  route_nn_list=route_nn_list)
                    except Exception:
                        # best-effort fallback for this cell
                        pass
                x_np = np.stack([np.full(n_bins, cell_id, dtype=np.int64), bins_global, bins_global], axis=1)
                x = torch.from_numpy(x_np).long().to(device_local)
                x_chrom = torch.from_numpy(chrom_idx_arr).long().to(device_local)
                with torch.no_grad():
                    # EncoderLayer expects chroms_in_batch to be a 1-length iterable
                    out_tuple = self.higashi_model(x, x_chrom, chroms_in_batch=[c+1])
                    try:
                        _, _, _, atac_b1, _ = out_tuple
                    except Exception:
            ...
            if want_prob or want_raw:
                # Prepare per-bin statistics on demand
                mu = sigma = None
                if g is not None and chrom_arr is not None:
                    try:
                        mask = (chrom_arr == chrom)
                        # [cells, bins_on_chrom]
                        mats = []
        ...
        if h5 is not None:
            try:
                ...
                pass
        # restore dynamic encoder forward if altered
        try:
            if use_graphsage and hasattr(self.higashi_model.encode1.dynamic_nn, 'forward_on_hook'):
                self.higashi_model.encode1.dynamic_nn.forward = self.higashi_model.encode1.dynamic_nn.forward_on_hook
        except Exception:
            pass
    ...
    def fetch_cell_embeddings(self, refresh: bool = False):
        """Return cell embeddings (N_cells x d_model).
        - If `refresh=True`, recompute from current `node_embedding_init` and update disk cache.
        - Otherwise, return cached in-memory or last saved file under `embed_dir`.
        """
        if refresh:
            # Recompute and cache from current model state
            self.save_embeddings()
            return self.cell_embeddings
        
        if self.cell_embeddings is not None:
            return self.cell_embeddings
        # Load last saved embeddings if present
        print("Loading from last training results")
        self.cell_embeddings = np.load(os.path.join(self.embed_dir, f"{self.embedding_name}_0_origin.npy"))
        return self.cell_embeddings
