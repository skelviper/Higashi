...
def impute_all(config):
    ...
    # Optional joint co-assay imputation settings
    coassay_joint = bool(config.get('coassay_imputation', False))
    atac_gamma = float(config.get('coassay_imputation_gamma', 0.5))  # weight strength for ATAC-guided Hi-C
    atac_alpha = float(config.get('coassay_imputation_alpha', 1.0))  # blend for ATAC diffusion
    signal_file = None
    signal_name = None
    chrom_bin_mask = {}
    if coassay_joint:
        try:
            data_dir = config['data_dir']
            signal_names = config.get('coassay_signal', [])
            if len(signal_names) == 0:
                print('coassay_imputation=True but no coassay_signal specified; skipping joint imputation.')
                coassay_joint = False
            else:
                signal_name = signal_names[0]
                signal_file = h5py.File(os.path.join(data_dir, 'sc_signal.hdf5'), 'r')
                # Precompute per-chrom bin masks
                _chrom_ds = signal_file[signal_name]['bin']['chrom']
                try:
                ...
                for c in impute_list:
                    chrom_bin_mask[c] = (chrom_list_signal == c)
                print('Enabled coassay joint imputation using signal:', signal_name)
        except Exception as e:
            print('Failed to prepare coassay joint imputation:', str(e))
            coassay_joint = False
    ...
    with h5py.File(os.path.join(rw_dir, "schicluster.hdf5"), "w") as hicluster_f:
        for c in impute_list:
            ...
            with h5py.File(os.path.join(rw_dir, "rw_%s.hdf5" % c), "w") as f:
                ...
                # Optional: collect ATAC imputations per chromosome
                atac_imputed_list = [] if coassay_joint else None
                for i, m in enumerate(tqdm(a)):
                    A = np.array(m.todense())
                    # ATAC-guided Hi-C weighting
                    if coassay_joint:
                        try:
                            x_full = np.array(signal_file[signal_name][str(i)])
                            x_raw = x_full[chrom_bin_mask[c]].astype('float32')
                            # Normalize accessibility to [0,1] to compute pairwise weights
                            xmax = np.max(x_raw) + 1e-8
                            x_norm = x_raw / xmax
                            w = (1.0 + atac_gamma * (np.outer(x_norm, x_norm)))
                            A = A * w
                        except Exception as e:
                            # Fallback: proceed without ATAC guidance for this cell
                            pass
                    # Hi-C imputation via smoothing/random walk
                    M_imp = impute_gpu(A)
                    v = M_imp[samples[:, 0], samples[:, 1]]
                    f.create_dataset("cell_%d" % (i), data=v)
                    # ATAC diffusion using imputed Hi-C graph
                    if coassay_joint:
                        try:
                            # Reuse the propagated matrix as influence kernel
                            Q = torch.from_numpy(M_imp).float()
                            # Convert to device and normalize columns similar to random_walk_gpu
                            device_local = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                            Q = Q.to(device_local)
                            P = Q - torch.diag(torch.diag(Q))
                            P = torch.div(P, torch.sum(P, 0) + 1e-15)
                            # One-step propagation on ATAC vector
                            x = torch.from_numpy(x_raw).float().to(device_local)
                            x_prop = torch.matmul(P, x)
                            x_out = (1 - atac_alpha) * x + atac_alpha * x_prop
                            atac_imputed_list.append(x_out.detach().cpu().numpy())
                        except Exception:
                            pass
                # Save per-chromosome ATAC imputation if available
                if coassay_joint and atac_imputed_list is not None and len(atac_imputed_list) == len(a):
                    atac_imputed = np.stack(atac_imputed_list, axis=0)
                    np.save(os.path.join(rw_dir, f"rw_atac_{c}.npy"), atac_imputed)
...
def process_signal(config):
    ...
    for signal in signal_names:
        ...
        # Standardize values globally（不做重分箱）
        ...
        for chrom in chrom_list:
            mask = (chrom_list_signal == chrom)
            chrom2signals[chrom].append(one_signal_stack[:, mask])
